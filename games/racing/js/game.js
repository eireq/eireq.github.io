(() => {
  ...
  function spawnTraffic() {
    ...
    const choices = Array.from({ length: lanes }, (_, i) => i).filter(l => !occupied.has(l));
  ...
  function rectHit(a, b) {
    return Math.abs(a.x - b.x) < (a.width + b.width) * .39 &&
           Math.abs(a.y - b.y) < (a.height + b.height) * .38;
  }
  ...
  function update(dt) {
    ...
    if (state.mode === "bomb") {
      // the bomb becomes dangerous if the player refuses to keep moving fast.
      const required = 280 + state.difficulty * 90;
      if (state.speed < required) state.bomb -= (required - state.speed) * .018 * dt;
    ...
    state.distance += state.speed * dt * .075;
    state.score = Math.floor(state.distance * 10 + state.overtakes * 125 + state.bestSpeed * .35);

    state.roadOffset = (state.roadOffset + state.speed * dt) % 64;

    state.spawnEvery = Math.max(.62, 1.05 - state.difficulty * .25);
    state.spawnTimer -= dt;
    if (state.spawnTimer <= 0) {
      spawnTraffic();
      state.spawnTimer = state.spawnEvery * (.75 + Math.random() * .55);
    }
    ...
    for (const s of state.scenery) {
      ...
      if (s.y > H + 40) {
        s.y = -Math.random() * 100;
        s.side = Math.random() < .5 ? "left" : "right";
      }
    }
  ...
  function draw() {
    ...
    for (const p of state.particles) {
      ...
      ctx.fillStyle = p.life > .4 ? "#ffd84a" : "#ff4d4d";
  ...
  function loop(time) {
    ...
    const dt = Math.min(.033, (time - state.lastTime) / 1000);
    state.lastTime = time;
  ...
  window.addEventListener("keydown", e => {
    ...
    if (e.key.toLowerCase() === "p") state.running && state.paused !== undefined && window.game.togglePause();
  });
